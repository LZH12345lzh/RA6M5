...
void QSPI_FLASH_BufferWrite(uint8_t *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
    ...
    if (Addr == 0)
    {
        ...
        if (NumOfPage == 0)
        {
            R_QSPI_Write(&Flash_ctrl, pBuffer, (uint8_t *)WriteAddr, NumByteToWrite);
            get_flash_status();
        ...
        else /* NumByteToWrite > SPI_FLASH_PageSize */ 
        {
            ...
            while (NumOfPage--)
            {
                R_QSPI_Write(&Flash_ctrl, pBuffer, (uint8_t *)WriteAddr, SPI_FLASH_PageSize);
                get_flash_status();
            ...
            /*若有多余的不满一页的数据，把它写完*/
            R_QSPI_Write(&Flash_ctrl, pBuffer, (uint8_t *)WriteAddr, NumOfSingle);
            get_flash_status();
    ...
    else
    {
        ...
        if (NumOfPage == 0)
        {
            ...
            if (NumOfSingle > count)
            {
                ...
                /*先写满当前页*/
                R_QSPI_Write(&Flash_ctrl, pBuffer, (uint8_t *)WriteAddr, count);
                get_flash_status();
                ...
                pBuffer += count;
                /*再写剩余的数据*/
                R_QSPI_Write(&Flash_ctrl, pBuffer, (uint8_t *)WriteAddr, temp);
                get_flash_status();
...
{
    R_QSPI_Read(&Flash_ctrl, pBuffer, (uint8_t *)ReadAddr, NumByteToRead);
}
